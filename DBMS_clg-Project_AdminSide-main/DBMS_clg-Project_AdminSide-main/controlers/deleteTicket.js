...
function deleteTicket(req) {
    ...
    return new Promise((resolve, reject) => {
        connection.query(
            ...
            (UserId, TicketId, Fare, DateOfBoarding, Email, numebr, nameOfPassenger , Boardind_From , Boardind_To , TrainId) 
            select UserId , TicketId , Fare , DateOfBoarding, Email , numebr , nameOfPassenger , Boardind_From , Boardind_To , TrainId from Tickets 
...
function getDeleteTicket(req, res) {
    deleteTicket(req)
        .then(() => {
            ...
            res.redirect(`/passenger/${req.params.tno}`);
            return;
        })
        .catch((err) => {
            console.log(err);
